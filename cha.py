...
# Function to clean and extract only valid Python code
def clean_llm_code(text: str) -> str:
    # Extract all code blocks (```...```) or fallback to full text
    code_blocks = re.findall(r"```(?:python)?(.*?)```", text, re.DOTALL)
    code = "\n".join(code_blocks).strip() if code_blocks else text

    # Remove any remaining explanation lines (heuristically)
    lines = code.splitlines()
    filtered_lines = []
    for line in lines:
        if re.match(r"^\s*(#|import|from|df|plt|result|print|for|if|def|class|with|return|pd|[a-zA-Z0-9_]+\s*=)", line.strip()):
            filtered_lines.append(line)
        elif line.strip() == "":
            filtered_lines.append("")  # preserve spacing
    code = "\n".join(filtered_lines)
